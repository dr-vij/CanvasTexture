...
namespace ViJApps.TextureGraph.ThirdParty
{
    ...
    public static class Converters
    {
        ...
        /// <param name="mult"></param>
        /// <returns></returns>
        public  static Path64 Float2ToPath64(IEnumerable<float2> position, double mult = DefaultMult)
            => new Path64(position.Select(c=> new Point64(c.x * mult, c.y * mult)));
        ...
        public static ContourVertex[] Path64ToContourVertexArr(Path64 positions, double mult = DefaultMult)
        {
            ...
            for (int i = 0; i < positions.Count; i++)
                result[i] = new ContourVertex(new Vec3((float)(positions[i].X / mult), (float)(positions[i].Y / mult), 0f));
            return result;
        }
